...
gray = cv2.cvtColor(cv2.imread(Cropped_loc), cv2.COLOR_BGR2GRAY)
ret, out = cv2.threshold(gray, 125, 255, cv2.THRESH_BINARY)  


# cv2.imshow("cropped",cv2.imread(Cropped_loc)) 
cv2.imshow("cropped",out) 
text=pytesseract.image_to_string(out,lang='eng') #converts image characters to string
